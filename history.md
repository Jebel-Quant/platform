...
With the kitchen in place, the work of building strategies can begin. Each strategy lives in its own repository — but left unmanaged, a collection of strategy repos rapidly becomes a zoo. CI workflows diverge. Python versions drift. Linting configs split. A security fix lands in one repo and is quietly missed by the rest. The same fragmentation that plagued the old world of personal scripts reappears at the infrastructure level.

The answer is [Rhiza](https://github.com/Jebel-Quant/rhiza-education) — a living template system developed at Jebel-Quant. Rather than generating project scaffolding once at day zero and walking away, Rhiza keeps every strategy repository continuously aligned with the platform's canonical standards. When the central template evolves — a new CI workflow, an updated linting config, a change to the containerisation setup — Rhiza opens a pull request in each downstream repo with a clean diff of what changed. Owners review, adapt if needed, and merge. Nothing is forced; nothing is missed.

This is the infrastructure equivalent of the shared tooling principle. Just as researchers should not be reimplementing portfolio construction from scratch, developers should not be manually maintaining CI pipelines in every repo. Rhiza ensures that the scaffolding around each strategy stays consistent with the platform, so the team's attention stays on what is inside the scaffolding.

## Live Trading

The moment a strategy goes live, the cost of a discrepancy between research and production becomes real. A backtest that behaves differently from the live system — because of an environment mismatch, a subtle difference in data handling, or a parameter that was hardcoded in research and misconfigured in production — can produce losses that no amount of prior testing would have predicted. Closing this gap is not a nice-to-have; it is a core design requirement.

The answer lies in proximity. The container that a researcher uses to develop and backtest a strategy is the same container that runs in production. Not a reimplementation, not a port, not an approximation — the same artifact. There is no translation step where something can go wrong, because there is no translation step.

What changes between environments is not the code — it is the configuration. The container is a fixed, versioned artifact; it consumes configuration files that tell it where to find data, which parameters to use, what risk limits to respect, and which execution venue to connect to. Moving a strategy from backtesting to paper trading to live is a matter of changing the configuration it runs against, not rebuilding the system. The container does not know whether it is in research or production. It only knows what its configuration tells it.

This separation of code from configuration is what makes promotion between environments safe and auditable. Every configuration file is versioned. Every deployment is a combination of a known container image and a known configuration state. If something goes wrong in production, the team can reconstruct exactly what was running, when, and with what parameters — and the path back to a known-good state is a configuration change, not an emergency code deployment.

**Prime broker connectivity.** A live strategy ultimately has to communicate with the outside world — placing orders, receiving fills, querying positions, and reconciling cash and margin with a prime broker. This boundary is one of the most consequential in the entire system. Errors here are not silent; they are immediate and financial.

The platform abstracts prime broker communication behind a clean interface. The strategy expresses intent — buy this instrument, in this quantity, with this urgency — and the execution layer translates that into the specific protocol the broker expects, whether FIX, a proprietary API, or otherwise. The strategy does not need to know which broker it is talking to, and switching brokers or adding a new venue is a configuration change rather than a code change.

This abstraction also means that in backtesting and paper trading, the same execution interface is present — backed by a simulated fill engine rather than a live connection. The strategy code is identical in all environments. The broker, like everything else, is a configuration detail.
